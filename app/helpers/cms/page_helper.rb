    ...
    def render_breadcrumbs(options={})
      ...
      ancestors[start..ancestors.size].each_with_index do |sec,i|
        items << content_tag(:li, 
          link_to(sec.name, sec.actual_path), (i == 0 ? {:class => options[:first_class] || "first"} : i == ancestors.size ? {:class=>options[:active_class] || "uk-active"} : {}))
      end
      ...
      content_tag(:ul, "\n  #{items.join("\n  ")}\n".html_safe, :class => options[:class]||"breadcrumbs")
    end
